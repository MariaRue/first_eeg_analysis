...
if isstruct(data) % session data 
    ...
    for j = 1 : num_sessions % loop through sessions 
        ...
        winning = zeros(num_trials,1); % set up winning vector 
        loosing = zeros(num_trials,1); % set up loosign vector 
        
        
        
       
        for i = 2 : num_trials % loop through trials 
        ...
        winleft = [0; winning(2:end).*(1-data(j).data(1:end-1,2))]; %won on left on t-1?
        winright = [0; winning(2:end).*data(j).data(1:end-1,2)]; % won on right on t-1?
        choseright = [0; data(j).data(1:end-1,2)]; %chose right on t-1?
        Y = data(j).data(:,2); % column indicating rightward choice

        X = [ones(num_trials,1),data(j).data(:,4), winleft, winright];
        %first column is constant, second column list with coherences, 3rd column
        %winning vector, 4th loosing vector
        
        %to do split half analyses:
        Xeven = X(2:2:end,:); Xodd = X(3:2:end,:);
        Yeven = Y(2:2:end); Yodd = Y(3:2:end);
        
        [PMF_fit(j,:),~,logit_stats{j}] = glmfit(X,Y,'binomial','link','logit','constant','off'); %logistic fit parameters
        [PMF_fit_even(j,:),~,logit_stats_even{j}] = glmfit(Xeven,Yeven,'binomial','link','logit','constant','off'); %logistic fit parameters
        [PMF_fit_odd(j,:),~,logit_stats_odd{j}] = glmfit(Xodd,Yodd,'binomial','link','logit','constant','off'); %logistic fit parameters
...
%         param.b3 =  PMF_fit(j,4); % loosing 
        ...
        [percent_right,n,confidence_interval,rt,coherences] = PMF_data(data(j).data);
