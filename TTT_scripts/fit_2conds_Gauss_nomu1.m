function L = fit_2conds_Gauss_nomu1(params, logit)
...
% will use. The function fits just one PMF to the input data, which has two
...
%
% [X, fval, exitflag,funcCount] = fminsearch(@fit_2conds_Gauss_nomu1, params,
% options, logit)
%
%
...
mu = params(1); % Mean for both conditions
%mu_1 = params(2); % Change in mean for left_bias condition

sigma = params(2); % SD for both conditions
...
y(idx_2) = (1/2)*(1 + erf(([logit.x(idx_1)] - mu)./(sigma*sqrt(2))));
