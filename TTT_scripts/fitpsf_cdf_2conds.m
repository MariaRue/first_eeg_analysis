function [mu, sd] = fitpsf_cdf_2conds(probitin, varargin)
%
% The function
%       [mu, sd] = fitpsf_cdf_2conds(probitin, varargin)
%
% fits a culmulative Gaussian to binomial data. The data is in a structure
% 'probit' which has:
%
% - probit.x = the stimulus difficulty level (for example, disparity or
% motion coherence) ;
% - probit.n = number of stimulus presentations of each stimulus ;
% - probit.resps = the number of responses in the +ve direction (check
% conventions) ;
% - probit.expno = ID number of the experimental condition (reward or
% social bias).
%
% expno == 1 : rightwards social cue
% expno == 2 : leftwards social cue. 
%
%
%
%
% Nela Cicmil, 15th November 2016, University of Oxford
% Edited 23rd December, 2016, University of Oxford.
%
% Based on previous scripts "megreward_fitpsf_cdf.m" and "basic_fitpsf_cdf.m".
% (Originally based on Bruce Cumming's script "fitpsf.m". His code is still in there.)
...
% LEFTWARDS social cue condition: ORANGE
if expno == 2
    
   orange = [1 0.5 0];
   errorbar(stim_set, probit.resps ./ probit.n, probit.sem,'o', 'Color', orange, 'MarkerFaceColor', orange,  'LineWidth', 1.5);
   hold on;
...
% RIGHTWARDS social cue condition: BLUE
elseif expno == 1    
  
  errorbar(stim_set, probit.resps ./ probit.n, probit.sem, 'o', 'color', 'b', 'MarkerFaceColor', 'b',  'LineWidth', 1.5);  
  hold on;

  g_cdf = 0.5 + erf((g_stim - mu)/(sd * sqrt(2)))/2;
  plot(g_stim, g_cdf, '-', 'color', 'b', 'LineWidth', 1.5);  
  hold on;
