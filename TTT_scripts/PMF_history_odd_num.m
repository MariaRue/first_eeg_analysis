function [logit_stats] = PMF_history_odd_num(data,task)
...
if isstruct(data) % session data 
    ...
    for j = 1 : num_sessions % loop through sessions 
        ...
        winning = zeros(ceil(num_trials/2)-1,1); % set up winning vector 
        loosing = zeros(ceil(num_trials/2)-1,1); % set up loosign vector 
        
        
        
        z = 1;
        
        for i = 3 :2 : num_trials % loop through trials 
        ...
        winleft = [winning.*(1-data(j).data(2:2:end-1,2))]; %won on left on t-1?
        winright =[winning.*   data(j).data(2:2:end-1,2) ]; % won on right on t-1?
        %choseright = [data(j).data(1:end-1,2)]; %chose right on t-1?
        Y = data(j).data(3:2:end,2); % column indicating rightward choice
        
        X = [ones((ceil(num_trials/2)-1),1),data(j).data(3:2:end,4), winleft, winright];
        ...
        [percent_right,n,confidence_interval,rt,coherences] = PMF_data(data(j).data(1:2:end,:));
        ...
        hold on
        plot(xrange,p.*100,'r-'); % fitted PMF
        errorbar(coherences,percent_right,confidence_interval,'ro') % data
        hold off
        ylim([0 100]);
